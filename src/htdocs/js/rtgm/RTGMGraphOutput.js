...
RTGMGraphOutput.prototype._initialize = function () {
  this.el.classList.add('rtgm-graph-output');

  this._hazardGraphOutput = this.el.appendChild(
      document.createElement('div'));
  this._hazardGraphOutput.classList.add('graph');

  this._cdfGraphOutput = this.el.appendChild(
      document.createElement('div'));
  this._cdfGraphOutput.classList.add('graph');

  this._pdfGraphOutput = this.el.appendChild(
      document.createElement('div'));
  this._pdfGraphOutput.classList.add('graph');
  this._pdfGraphOutput.classList.add('page-break');

  this._integrandGraphOutput = this.el.appendChild(
      document.createElement('div'));
  this._integrandGraphOutput.classList.add('graph');

  this._riskGraphOutput = this.el.appendChild(
      document.createElement('div'));
  this._riskGraphOutput.classList.add('graph');

  this._legend = document.createElement('div');

  this._collection.on('select', RTGMGraphOutput.prototype.render, this);
};
  ...
  new Dygraph(this._hazardGraphOutput, dataStr.join('\n'), Util.extend({},
    ...
    drawPointCallback: function (g, name, ctx, cx, cy, color, sz, idx) {
      if (idx < oMinIdx || idx > oMaxIdx) {
        return Dygraph.Circles.DEFAULT(g, name, ctx, cx, cy, color, 2*sz, idx);
      } else {
...
module.exports = RTGMGraphOutput;
